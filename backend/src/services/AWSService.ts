...
export class AWSService {
  ...
  async listAPIGatewayAPIs(params: {
    ...
    try {
      ...
      if (this.credentialService.shouldUseLocalCredentials()) {
        ...
        // Create API Gateway client with local credentials
      ...
      logger.error('Failed to list API Gateway APIs', { 
        accountId: params.accountId,
        error: error.message
      })
      
      throw createError(`Failed to list API Gateway APIs: ${error.message}`, 500)
    }
  }
  ...
  async listAPIGatewayStages(params: {
    ...
    try {
      ...
      this.credentialService.auditCredentialUsage(params.accountId, 'apigateway:GET', false)
      logger.error('Failed to list API Gateway stages', { 
        accountId: params.accountId,
        apiId: params.apiId,
        error: error.message
      })
      ...
      throw createError(`Failed to list API Gateway stages: ${error.message}`, 500)
    }
  }
  ...
  async analyzeAPIGateway(params: {
    ...
    try {
      if (params.operation === 'access_logs') {
        ...
        const logResult = await this.queryCloudWatchLogs({
          accountId: params.accountId,
          logGroup: params.logGroup,
          startTime: params.startTime || Date.now() - 24 * 60 * 60 * 1000,
          endTime: params.endTime || Date.now(),
